...
class EchoStateNetwork:
    ...
    def _RVC_Noise(x, case, U_wtv, Y_wtv, tikh_opt, hp_names, print_convergence=True):
        ...
        See Also
        --------
        _SSV, _WFV, _KFV : the single-series validation strategies of
            Racca & Magri (2021) (single shot, walk forward, K-fold),
            available for comparison on a single contiguous training series.
        _SegmentRVC_Noise : recycle validation for a ragged corpus of segments.
        """
    ...
    @staticmethod
    def _SegmentRVC_Noise(x, case, U_wtv, Y_wtv, tikh_opt, hp_names, print_convergence=True):
        ...
        *list* of segments (the ragged path in _split_and_format_data), not a
        regular ndarray -- for a single long trajectory, use _RVC_Noise instead.

        _RVC_Noise carves several within-segment folds out of each segment, spaced
        ...
        _RVC_Noise's own comment). Neither is a good hyperparameter signal.
        ...
        for l in probeable:
            ...
            for tik_j in range(N_tikh):
                ...
                # per-probe penalty for a diverged run, as in _RVC_Noise (never
    ...
    @staticmethod
    def _single_series_validation(x, case, U_wtv, Y_wtv, tikh_opt, hp_names,
                                  print_convergence, folds_of, strategy_name):
        """Shared engine for the single-series validation strategies of
        Racca & Magri (2021): `_SSV`, `_WFV` and `_KFV` differ only in fold
        ...
        Noise handling is identical to `_RVC_Noise`: U_wtv is already the noisy
        ...
        x, case, U_wtv, Y_wtv, tikh_opt, hp_names, print_convergence
            As in `_RVC_Noise`, except U_wtv/Y_wtv must be regular ndarrays with
        ...
        strategy_name : str
            Name used in error messages ('_SSV', '_WFV', '_KFV').
        ...
        if not isinstance(U_wtv, np.ndarray) or U_wtv.ndim != 3 or U_wtv.shape[0] != 1:
            raise ValueError(
                ...
                'Use _RVC_Noise or _SegmentRVC_Noise instead.')
        ...
        if case.input_parameters is not None:
            ...
            # closed-loop probes (same slice trick as _RVC_Noise; train()
        ...
        for train_ranges, i0, n_val_k in folds:
            ...
            for tik_j in range(N_tikh):
                ...
                # washed state -- equivalent to _RVC_Noise's washout inside the
                ...
                # Per-fold penalty for a diverged closed loop, as in _RVC_Noise
                # (never reset the accumulated sum -- that erases the other
                # folds' signal).
                err = np.log10(case.compute_nRMSE(Y_val, Y_closed, norm=norm_l))
                nRMSE[tik_j] += err if np.isfinite(err) else 10.0

        # select and save the optimal tikhonov (same bookkeeping as _RVC_Noise:
    ...
    @staticmethod
    def _SSV(x, case, U_wtv, Y_wtv, tikh_opt, hp_names, print_convergence=True):
        ...
        the multi-interval strategies (`_WFV`, `_KFV`, `_RVC_Noise`).
        ...
        U_wtv : np.ndarray
            ...
            ValueError (use `_RVC_Noise` or `_SegmentRVC_Noise` for those).
        Y_wtv : np.ndarray
        ...
        return EchoStateNetwork._single_series_validation(
            x, case, U_wtv, Y_wtv, tikh_opt, hp_names, print_convergence,
            folds_of, '_SSV')

    @staticmethod
    def _WFV(x, case, U_wtv, Y_wtv, tikh_opt, hp_names, print_convergence=True):
        ...
        the folds, which is far more robust for chaotic series than `_SSV`.
        ...
        def folds_of(case_, n_post):
            ...
            if K < case_.N_folds and case_.val_k == 0:
                print(f'_WFV: only {K} of the requested N_folds={case_.N_folds} '
    ...
    @staticmethod
    def _KFV(x, case, U_wtv, Y_wtv, tikh_opt, hp_names, print_convergence=True):
        ...
        data that `_RVC_Noise` embraces for its washout windows. `_RVC_Noise`
        ...
        def folds_of(case_, n_post):
            ...
            if K < case_.N_folds and case_.val_k == 0:
                print(f'_KFV: only {K} of the requested N_folds={case_.N_folds} '
